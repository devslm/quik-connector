...
Добавлена сборка OpenSSL 1.1.1l (необходима для работы HTTP клиента). Включается в сборку отдельным флагом. Документация: [OpenSSL](https://www.openssl.org/).
...
2. Сборка OpenSSL библиотеки **опционально**
3. Сборка DLL библиотеки
4. Установка Redis-сервера
...
### Сборка OpenSSL библиотеки (Опционально)
Для запуска сборки необходимо запустить консоль **Командная строка Native Tools x64**.
В ней перейти в каталог проекта **/path-to-quik-connector/external/openssl-1.1.1l** и выполнить:
```shell
build.bat
```

Будет выполнена сборка необходимых библиотек. После компиляции в каталоге **external/openssl-1.1.1l/build** появятся необходимые **.lib** файлы.
Когда будет полностью собран весь проект, то в каталоге собранного коннектора **quik-connector/quik-connector/bin** появятся 2 **.dll** файла:
- **libcrypto-1_1-x64.dll**
- **libssl-1_1-x64.dll**

Данные файлы необходимо скопировать в каталог терминала **QUIK**, иначе будет ошибка при запуске коннектора. Если поддержка OpenSSL выключена, то ничего копировать
не нужно.
...
cmake -DCMAKE_BUILD_TYPE=Release -DUSE_LUA_VERSION_5_4=ON -DQUIK_INCLUDE_TA_LIB=OFF -DQUIK_INCLUDE_OPENSSL=OFF .. -A x64
...
- QUIK_INCLUDE_OPENSSL - если ON то включить поддержку OpenSSL, иначе не подключается и не требуется собирать библиотеку
...
```shell
  |-- bin
  |    |-- libcrypto-1_1-x64.dll (если включена поддержка OpenSSL)
  |    |-- libssl-1_1-x64.dll (если включена поддержка OpenSSL)
  |    |-- quik-connector.dll
  |  
  |-- config
  |    |-- config.yml
...
В обоих случаях в поле id с типом UUID записываться может:
  ...
  - errors - список ошибок (например, при валидации на каждое не валидное поле может быть ошибка и т.п.)
